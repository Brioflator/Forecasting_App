...
// Vendored from animate-ui (https://animate-ui.com/r/primitives-texts-counting-number.json).
// Local deviation: an optional `formatNumber` prop so KPI values can render
// with locale thousand separators (the upstream primitive only supports
// decimal-separator / pad-start formatting).
...
function CountingNumber({
  ...
  React.useEffect(() => {
    const unsubscribe = springVal.on('change', (latest) => {
      if (localRef.current) {
        if (formatNumber) {
          localRef.current.textContent = formatNumber(latest);
          return;
        }
        let formatted =
          decimals > 0
    ...
    return () => unsubscribe();
  }, [springVal, decimals, padStart, number, decimalSeparator, localRef, formatNumber]);
